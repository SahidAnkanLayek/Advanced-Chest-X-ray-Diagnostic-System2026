...
import { Upload, X, Loader2, Thermometer, FileDown, Eye, EyeOff, AlertCircle, Info, Zap, User, Settings2, Palette } from 'lucide-react';
import PredictionList from './PredictionList';
import { analyzeXRay, downloadReport } from '../services/api';
import { PredictionResponse, PatientDetails } from '../types';
...
const XRayLab: React.FC = () => {
  ...
  const runAnalysis = async () => {
    ...
    setIsAnalyzing(true);
    setAnalysisStep('Connecting to Neural Engine API...');
    
    try {
      await new Promise(r => setTimeout(r, 800));
      setAnalysisStep('HuggingFace Inference Running (CheXNet DenseNet-121)...');
      
      const response = await analyzeXRay(file, patient);
      setResult(response);

    } catch (err) {
      console.error(err);
      alert('Analysis failed. Please ensure the Render backend is active.');
    } finally {
      setIsAnalyzing(false);
      setAnalysisStep('');
    }
  ...
  const handleDownloadReport = () => {
    ...
    downloadReport(result.pdf_base64, result.pdf_filename);
  };
      ...
      <div className="flex items-center justify-between">
        ...
        {result && (
          <div className="flex gap-2">
            <button 
              onClick={handleReset}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors text-sm"
            >
              <X size={16} /> New Scan
            </button>
            <button 
              onClick={handleDownloadReport}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-500 transition-colors text-sm shadow-lg shadow-blue-600/20"
            >
          ...
          <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden aspect-square relative group shadow-sm">
            {!preview ? (
              <label className="absolute inset-0 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                ...
                <input 
                  type="file" className="hidden" ref={fileInputRef} onChange={handleFileChange} accept="image/*"
                />
              </label>
            ) : (
              <div className="relative w-full h-full flex items-center justify-center bg-slate-50 dark:bg-black">
                {showHeatmap && result ? (
                  <img 
                    src={result.heatmap_base64} alt="Grad-CAM Overlay" className="max-h-full max-w-full object-contain"
                    style={{ opacity: heatmapOpacity, filter: `hue-rotate(${heatmapHue}deg)` }}
                  />
                ) : (
                  <img src={preview} alt="X-ray Preview" className="max-h-full max-w-full object-contain" />
                )}
          ...
          <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input type="text" value={patient.name} onChange={(e) => setPatient({...patient, name: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] text-slate-500 uppercase">Patient ID</label>
                <input type="text" value={patient.patientId} onChange={(e) => setPatient({...patient, patientId: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] text-slate-500 uppercase">Date of Birth</label>
                <input type="date" value={patient.dob} onChange={(e) => setPatient({...patient, dob: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] text-slate-500 uppercase">Gender</label>
                <select value={patient.gender} onChange={(e) => setPatient({...patient, gender: e.target.value as any})} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          {!result && !isAnalyzing ? (
            <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 p-8 rounded-2xl text-center flex-1 flex flex-col items-center justify-center shadow-sm">
              ...
              <button 
                disabled={!file} onClick={runAnalysis}
                className={`w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${file ? 'bg-blue-600 text-white hover:bg-blue-500 shadow-lg shadow-blue-600/20' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-600 cursor-not-allowed'}`}
              >
                    ...
                    <div className="space-y-3">
                      <div className="space-y-1">
                        <div className="flex justify-between text-[10px] font-bold text-slate-500">
                          <span>OPACITY</span><span>{Math.round(heatmapOpacity * 100)}%</span>
                        </div>
                        <input type="range" min="0.1" max="1.0" step="0.05" value={heatmapOpacity} onChange={(e) => setHeatmapOpacity(parseFloat(e.target.value))} className="w-full h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full appearance-none cursor-pointer accent-blue-600" />
                      </div>

                      <div className="space-y-1">
                        <div className="flex justify-between text-[10px] font-bold text-slate-500">
                          <div className="flex items-center gap-1"><Palette size={10} /><span>COLOR GRADIENT (HUE)</span></div>
                          <span>{heatmapHue}°</span>
                        </div>
                        <input type="range" min="0" max="360" step="1" value={heatmapHue} onChange={(e) => setHeatmapHue(parseInt(e.target.value))} className="w-full h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full appearance-none cursor-pointer accent-green-500" />
                      </div>
                    </div>
                  </div>
                )}
                <PredictionList predictions={result.probabilities} />
              </div>

              <div className="bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-2xl flex gap-4">
                <div className="mt-1"><AlertCircle className="text-emerald-600 dark:text-emerald-500" size={20} /></div>
                <div className="text-sm">
                  <p className="text-emerald-700 dark:text-emerald-200 font-semibold mb-1">System Status: Active Backend</p>
                  <p className="text-emerald-600 dark:text-emerald-500/80 leading-relaxed">
                    Analysis performed successfully via FastAPI and DenseNet-121 neural engine. Please correlate findings with clinical history.
                  </p>
                </div>
              </div>
            </div>
