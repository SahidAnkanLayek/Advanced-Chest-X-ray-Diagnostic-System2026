...
const PredictionList: React.FC<PredictionListProps> = ({ predictions }) => {
  // FIX: We explicitly cast Object.entries to an array of [string, number] tuples.
  // This tells TypeScript: "Trust me, the second item is definitely a number."
  const sortedPredictions = (Object.entries(predictions) as [string, number][])
    .sort((a, b) => b[1] - a[1])
    .slice(0, 7);

  return (
    <div className="space-y-4">
      {/* Now we can safely destructure because TypeScript knows probability is a number */}
