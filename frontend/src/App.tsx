...
const App: React.FC = () => {
  ...
  const [authView, setAuthView] = useState<'login' | 'register' | 'verify' | 'forgotPassword'>('login');
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      // Access Rule: Only verified users can access ChestScan AI
      ...
      case AppTab.SETTINGS:
        return (
          <div className="max-w-2xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 text-center shadow-sm">
               <SettingsIcon size={32} className="mx-auto mb-3 text-slate-300 dark:text-slate-700" />
               <p className="text-sm text-slate-500 dark:text-slate-400">More configuration options are available in the Enterprise Dashboard.</p>
            </div>
          </div>
        );
      default:
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 overflow-hidden text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <aside className="w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col">
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 space-y-2">
          ...
          <div className="mt-4 p-3 bg-slate-100 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700/50">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-600 dark:text-blue-400 font-bold text-xs">
                {user?.displayName ? user.displayName.split(' ').map(n => n[0]).join('') : 'DR'}
              </div>
              <div className="flex-1 overflow-hidden">
                <p className="text-sm font-medium text-slate-900 dark:text-white truncate">{user?.displayName || 'Physician'}</p>
